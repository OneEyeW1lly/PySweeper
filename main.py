import pygame, math, random, ctypes
import os, sys
...
class MineField:
    ...
    def get_flag_count(self):
        return self.bomb_limit-len(self.flags)
...
class Game:
    def __init__(self) -> None:
        ...
        self.FPS:int = 20
        pygame.init()
        #self.grid_surface = pygame.Surface((1+SPRITE_SIZE*16, 1+SPRITE_SIZE*16))
        ...
        self.border = SpriteSheet("assets/border.png").image_at(pygame.Rect(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT))
        self.face_sheet = SpriteSheet("assets/faces.png")
        ...
        self.digits_sheet = SpriteSheet("assets/numbers_shadow.png")
    ...
    def game_timer_update(self) -> int:
        ...
        elif (is_active == False) and (self.grid.first_click == True):
            print('timer created!')
            self.timer.activate()
            return 0
        else:
    ...
    def end_game(self, final_bomb_index:int, message:str, style='ok'):
        self.grid.first_click = False
        self.running = False
        self.is_dead = True

        self.timer.deactivate()
        self.grid.reveal_bombs(final_bomb_index)
        self.update()
    ...
    def start(self) -> None:
        ...
        while (self.running):
            ...
            if win_check == 1:
                self.is_win = True
                self.end_game(7, "YOU WON! :)")
            elif win_check == -1:
                self.is_lose = True
                self.end_game(6, "YOU LOST! :(")
